...
2. **Switch to Docker deployment** (not Buildpack):
   - Set **Dockerfile path** to `Dockerfile.koyeb`
   - This uses `requirements-deploy.txt` (~300MB image vs ~4.5GB with torch)
...
4. **Health Check**: Set to `GET /api/health`.
